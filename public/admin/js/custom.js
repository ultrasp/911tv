...
var editor_config = {
  ...
  tinymce.init(editor_config);
   ...
   $('.add_input').on('click',function(){
      var new_input = $('.first').clone().removeClass('first');
      var number = $('.posts').length+1;
      var classname = 'tinymce'+number;
      new_input.find('.content_wrapper').empty().append('<textarea name="content[]" class="form-control summernote '+classname+'"></textarea>');
      //new_input.find('.summernote').summernote('code','');
      new_input.find('.control-label').append('<button class="remove_input btn btn-primary" type="button">Remove step to instruction</button>');
      new_input.find('.step_number').html($('.posts').length+1);
      $('.steps').append(new_input);
      editor_config.selector = '.'+classname;
      tinymce.init(editor_config);
   ...
   $("body").on('click','.totop',function(){
      var button = $(this);
      var wrapper = button.closest('.posts');
      var prev_wrapper= wrapper.prev('div');
      var number = parseInt(wrapper.find('.step_number').html()); 
      if(prev_wrapper.hasClass('posts') ){
         wrapper.insertBefore(prev_wrapper);
         wrapper.find('.step_number').html(number-1);
         prev_wrapper.find('.step_number').html(number);
      }
   })

   $("body").on('click','.tobottom',function(){
      var button = $(this);
      var wrapper = button.closest('.posts');
      var next_wrapper= wrapper.next('div');
      var number = parseInt(wrapper.find('.step_number').html()); 
      if(next_wrapper.hasClass('posts') ){
         next_wrapper.insertBefore(wrapper);
         wrapper.find('.step_number').html(number+1);
         next_wrapper.find('.step_number').html(number);
      }
   })
