...
class UserController extends Controller
{
    ...
    public function save_mac(Request $request){
        $request->validate(Macadres::$rules);
        $user = Auth::user();
        $mac = ($user->mac) ? $user->mac : new Macadres(['user_id' => Auth::id()]);
        $mac->mac = $request->input('mac');
        $is_new = $mac->exists; 
        if($mac->saveInApi()){
            $mac->save();
            return 1;
